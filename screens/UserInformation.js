...
const UserInformation = () => {
  ...
  useEffect(() => {
    // Retrieve matricNo from AsyncStorage if available
    const getMatricNo = async () => {
      try {
        const user = await AsyncStorage.getItem('userData');
        if (user) {
          const userData = JSON.parse(user);
          setMatricNo(userData.matricNo);
        }
      } catch (error) {
        console.error('Failed to load matric number', error);
      }
    };
  ...
  const handleSubmit = async () => {
    try {
      const response = await fetch("http://192.168.58.91:80/lcu-connect-2/user_information.php", {
        method: "POST",
        headers: {
      ...
      try {
        ...
        if (data.success) {
          ...
          navigation.navigate("HomePage"); // Navigate to ProfilePage after successful submission
        } else {
          alert(data.message);
        }
      } catch (error) {
